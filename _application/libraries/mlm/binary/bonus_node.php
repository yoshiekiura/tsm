...
class Bonus_node {
    ...
    public function execute() {
        $sql = "
            SELECT netgrow_node_network_id, 
            netgrow_node_level, 
            COUNT(*) AS node_count 
            FROM sys_netgrow_node 
            WHERE netgrow_node_date BETWEEN '" . $this->start_date . "' AND '" . $this->end_date . "' 
            AND netgrow_node_level <= " . $this->max_level . "
            GROUP BY netgrow_node_network_id, netgrow_node_level 
        ";
                ...
                if($bonus_amount > 0) {
                    ...
                    if($query_check->num_rows() > 0) {
                        $sql_update = "
                            ...
                            SET bonus_log_node = bonus_log_node + " . $bonus_amount . " 
                            WHERE bonus_log_network_id = '" . $network_id . "' 
                            AND bonus_log_date = '" . $this->log_date . "'
                        ";
