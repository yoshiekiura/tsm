...
class Systems_model extends CI_Model {
    ...
    function get_member_password($id) {
        $sql = "SELECT member_account_password FROM sys_member_account WHERE member_account_network_id = " . $id . "";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            $row = $query->row();
            return $row->member_account_password;
        } else {
            return '';
        }
    }
    ...
    function check_password($id, $password) {
        $sql = "SELECT 1 FROM sys_member_account WHERE member_account_password = '" . md5($password) . "' AND member_account_network_id = '" . $id . "'";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            return true;
        } else {
    ...
    function check_group_parent($network_id) {
        $sql = "SELECT 1 FROM sys_network_group WHERE network_group_parent_network_id = '" . $network_id . "'";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            return false;
        } else {
    ...
    function check_group_member($network_id) {
        $sql = "SELECT 1 FROM sys_network_group WHERE network_group_member_network_id = '" . $network_id . "'";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            return false;
        } else {
    ...
    function check_pin($network_id, $pin) {
        $sql = "
            SELECT 1 
            FROM sys_serial_user 
            INNER JOIN sys_serial ON serial_id = serial_user_serial_id 
            WHERE serial_user_network_id = '" . $network_id . "' 
            AND serial_pin = '" . $pin . "'
        ";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
    ...
    function delete_network_group($parent_network_id, $member_network_id) {
        $this->db->where('network_group_parent_network_id', $parent_network_id);
        $this->db->where('network_group_member_network_id', $member_network_id);
        $this->db->delete('sys_network_group');
    }
    
    function check_network_group($parent_group_network_id, $member_group_network_id) {
        $sql = "
            SELECT 1 
            FROM sys_network_group 
            WHERE network_group_parent_network_id = '" . $parent_group_network_id . "' 
            AND network_group_member_network_id = '" . $member_group_network_id . "' 
