<a class="btn btn-success btn-sm <?php if($position==FALSE){ echo 'disabled" disabled="disabled" href="#'; } else { echo '" href="' . base_url('voffice/network/node'); } ?>">Semua</a>
<a class="btn btn-success btn-sm <?php if($position=='left'){ echo 'disabled" disabled="disabled" href="#'; } else { echo '" href="' . base_url('voffice/network/node/left'); } ?>">Kiri</a>
<a class="btn btn-success btn-sm <?php if($position=='right'){ echo 'disabled" disabled="disabled" href="#'; } else { echo '" href="' . base_url('voffice/network/node/right'); } ?>">Kanan</a>
<br> <br>
<table id="gridview" style="display:none;"></table>

<form target="_blank" id="form_export" method="post" action="export_node_data">
...
<script>
    $("#gridview").flexigrid({
        ...
        dataType: 'json',
        colModel: [
            { display: 'Level', name: 'netgrow_node_level', width: 50, sortable: true, align: 'center' },
            { display: 'Jalur Kaki', name: 'netgrow_node_position_text', width: 90, sortable: true, align: 'center' },
            ...
            { display: 'Tanggal', name: 'netgrow_node_date', width: 120, sortable: true, align: 'center' },
        ],
        buttons_right: [
            { display: 'Export Excel', name: 'excel', bclass: 'excel', onpress: export_data },
        ],
        searchitems: [
            { display: 'Level', name: 'netgrow_node_level', type: 'text', isdefault: true },
            { display: 'Jalur Kaki', name: 'netgrow_node_position', type: 'select', option: 'L:Kiri|R:Kanan' },
            ...
            { display: 'Tanggal', name: 'netgrow_node_date', type: 'date' },
        ],
        sortname: "netgrow_node_id",
        sortorder: "desc",
        usepager: true,
        title: '<?php echo $title_data ?>',
        useRp: true,
        rp: 10,
