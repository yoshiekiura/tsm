...
/*
 * Backend Service Downloads Category Controller
 *
...
class Backend_service extends Backend_Service_Controller {
    ...
    function act_show() {
        ...
        if ($this->input->post('delete') != FALSE) {
            ...
            if (is_array($arr_item)) {
                ...
                foreach ($arr_item as $id) {
                    $check_delete = $this->backend_news_category_model->check_delete($id);
                    if ($check_delete) {
                        $this->function_lib->delete_data('site_news_category', 'news_category_id', $id);
                        $item_deleted++;
                    } else {
                        $item_undeleted++;
                    }
        ...
        if ($this->input->post('publish') != FALSE) {
            ...
            if (is_array($arr_item)) {
                foreach ($arr_item as $id) {
                    $data = array();
                    $data['news_category_is_active'] = '1';
                    $this->function_lib->update_data('site_news_category', 'news_category_id', $id, $data);
                }
        ...
        if ($this->input->post('unpublish') != FALSE) {
            ...
            if (is_array($arr_item)) {
                foreach ($arr_item as $id) {
                    $data = array();
                    $data['news_category_is_active'] = '0';
                    $this->function_lib->update_data('site_news_category', 'news_category_id', $id, $data);
                }
                $arr_output['message'] = 'Data berhasil disimpan.';
                $arr_output['message_class'] = 'response_confirmation alert alert-success';
    ...
    function get_data() {
        ...
        $params['table'] = "site_news_category";
        ...
        foreach ($query->result() as $row) {
            ...
            //is_active
            if ($row->news_category_is_active == '1') {
                $stat = 'Aktif';
            ...
            //edit
            $edit = '<a href="' . base_url() . 'backend/news_category/edit/' . $row->news_category_id . '"><img src="' . base_url() . _dir_icon . 'save_labled_edit.png" border="0" alt="Edit" title="Edit" /></a>';

            $entry = array('id' => $row->news_category_id,
                'cell' => array(
                    'news_category_id' => $row->news_category_id,
                    'news_category_title' => $row->news_category_title,
                    'news_category_is_active' => $is_active,
                    'edit' => $edit
                ),
            );
    ...
    function act_add() {
        ...
        $this->form_validation->set_rules('title', '<b>Nama Berita Kategori</b>', 'required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $news_category_title = $this->input->post('title');

            $data = array();
            $data['news_category_title'] = $news_category_title;
            $data['news_category_is_active'] = 1;
            $this->function_lib->insert_data('site_news_category', $data);
    ...
    function act_edit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $news_category_id = $this->input->post('id');
            $news_category_title = $this->input->post('title');

            $data = array();
            $data['news_category_title'] = $news_category_title;
            $this->function_lib->update_data('site_news_category', 'news_category_id', $news_category_id, $data);
