...
/*
 * Backend Service Region Controller
 *
...
class Backend_service extends Backend_Service_Controller {
    ...
    function act_show() {
        ...
        if ($this->input->post('delete') != FALSE) {
            ...
            if (is_array($arr_item)) {
                ...
                foreach ($arr_item as $id) {
                    $check_delete = $this->backend_region_model->check_delete($id);
                    if($check_delete == true) {
                        $this->function_lib->delete_data('ref_region', 'region_id', $id);
                        $item_deleted++;
                    } else {
                        $item_undeleted++;
                    }
        ...
        if ($this->input->post('publish') != FALSE) {
            ...
            if (is_array($arr_item)) {
                foreach ($arr_item as $id) {
                    $data = array();
                    $data['region_is_active'] = '1';
                    $this->function_lib->update_data('ref_region', 'region_id', $id, $data);
                }
    ...
    function get_data() {
        ...
        foreach ($query->result() as $row) {
            ...
            //edit
            $edit = '<a href="' . base_url() . 'backend/region/edit/' . $row->region_id . '"><img src="' . base_url() . _dir_icon . 'save_labled_edit.png" border="0" alt="Edit" title="Edit" /></a>';

            $entry = array('id' => $row->region_id,
                'cell' => array(
                    'region_id' => $row->region_id,
                    'region_name' => $row->region_name,
                    'region_is_active' => $is_active,
                    'edit' => $edit
                ),
            );
    ...
    function act_add() {
        ...
        $this->form_validation->set_rules('name', '<b>Nama Regional</b>', 'required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['region_is_active'] = 1;
            $this->function_lib->insert_data('ref_region', $data);
    ...
    function act_edit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array();
            $data['region_name'] = $region_name;
            $this->function_lib->update_data('ref_region', 'region_id', $region_id, $data);
