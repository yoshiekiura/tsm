...
class Backend_service extends Backend_Service_Controller {
        ...
        if ($this->input->post('delete') != FALSE) {
            ...
            if (is_array($arr_item)) {
                ...
                foreach ($arr_item as $id) {
                    ...
                    //hapus halaman
                    $this->function_lib->delete_data('site_page_member', 'page_id', $id);
        ...
        if ($this->input->post('publish') != FALSE) {
            ...
            if (is_array($arr_item)) {
                foreach ($arr_item as $id) {
                    ...
                    $this->function_lib->update_data('site_page_member', 'page_id', $id, $data);
    ...
    function get_data() {
        ...
        foreach ($query->result() as $row) {
            ...
            //edit
            $edit = '<a href="' . base_url() . 'backend/page_member/edit/' . $row->page_id . '"><img src="' . base_url() . _dir_icon . 'save_labled_edit.png" border="0" alt="Edit" title="Edit" /></a>';
    ...
    function act_add() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $page_id = $this->function_lib->insert_data('site_page_member', $data);
    ...
    function act_edit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['page_title'] = $page_title;
            $data['page_content'] = $page_content;
            $this->function_lib->update_data('site_page_member', 'page_id', $page_id, $data);
