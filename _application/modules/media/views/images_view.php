...
define('CACHE_DIR', $data_doc_root . '/' . $data_dir_cache);

// Images must be local files, so for convenience we strip the domain if it's there
$image = preg_replace('/^(s?f|ht)tps?:\/\/[^\/]+/i', '', (string) $data_images);
...
$docRoot = preg_replace('/\/$/', '', $data_doc_root);
...
$maxWidth = (isset($data_width)) ? (int) $data_width : 0;
$maxHeight = (isset($data_height)) ? (int) $data_height : 0;

if (isset($data_color))
    $color = preg_replace('/[^0-9a-fA-F]/', '', (string) $data_color);
else
    $color = FALSE;
...
if (isset($data_cropratio)) {
    $cropRatio = explode(':', (string) $data_cropratio);
...
$quality = (isset($data_quality)) ? (int) $data_quality : DEFAULT_QUALITY;
...
if (isset($data_cropratio))
    $resizedImageSource .= 'x' . (string) $data_cropratio;
...
if (!isset($data_nocache) && file_exists($resized)) {
