...
/*
 * Backend Service Modules Controller
 *
...
class Backend_service extends Backend_Service_Controller {
    ...
    function act_show() {
        ...
        if ($this->input->post('delete') != FALSE) {
            ...
            if (is_array($arr_item)) {
                ...
                foreach ($arr_item as $id) {
                    $this->function_lib->delete_data('site_modules', 'modules_id', $id);
                    $item_deleted++;
                }
        ...
        if ($this->input->post('publish') != FALSE) {
            ...
            if (is_array($arr_item)) {
                foreach ($arr_item as $id) {
                    $data = array();
                    $data['modules_is_active'] = '1';
                    $this->function_lib->update_data('site_modules', 'modules_id', $id, $data);
                }
    ...
    function get_data() {
        ...
        foreach ($query->result() as $row) {
            ...
            //edit
            $edit = '<a href="' . base_url() . 'backend/modules/edit/' . $row->modules_id . '"><img src="' . base_url() . _dir_icon . 'save_labled_edit.png" border="0" alt="Edit" title="Edit" /></a>';

            $entry = array('id' => $row->modules_id,
                'cell' => array(
                    'modules_id' => $row->modules_id,
                    'modules_name' => $row->modules_name,
                    'modules_links' => $row->modules_links,
                    'modules_is_active' => $is_active,
                    'edit' => $edit
                ),
            );
    ...
    function act_add() {
        ...
        $this->form_validation->set_rules('name', '<b>Nama Modul</b>', 'required');
        $this->form_validation->set_rules('links', '<b>Link Modul</b>', 'required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->session->set_flashdata('input_links', $this->input->post('links'));
            redirect($this->input->post('uri_string'));
        } else {
            $modules_name = $this->input->post('name');
            $modules_links = $this->input->post('links');

            $data = array();
            $data['modules_name'] = $modules_name;
            $data['modules_links'] = $modules_links;
            $data['modules_is_active'] = 1;
            $this->function_lib->insert_data('site_modules', $data);
    ...
    function act_edit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['modules_name'] = $modules_name;
            $data['modules_links'] = $modules_links;
            $this->function_lib->update_data('site_modules', 'modules_id', $modules_id, $data);
