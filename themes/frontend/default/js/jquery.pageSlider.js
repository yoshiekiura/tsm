...
(function($) {
    ...
    $.pageSlider = {
        ...
        defaults: {
            height: 1260,                // height of the slider panel
            width: 600,                 // width of the slider panel
            zIndex: 9000,               // z-index offset

            auto: false,                 // slide automaticly
            delay: 3000,                // delay between images in ms
            effect: 'horizontal',           // "random" => random effect or "vertical", "horizontal", ...
    ...
    $.pageSlider.addEffect('horizontal',
        function (current, next, opts) {
    ...
    function pageSlider(element, options) {
        ...
        function init() {
            ...
            $(element).css({
                ...
                //'overflow': 'hidden',
                'position': 'relative'
            }).wrap('<div class="pageSlider' + (theme !== null ? ' ' + theme : '') + '" id="pageSlider-' + element.id + '" />');
            wrapper = $('#pageSlider-' + element.id);
        ...
        function initTitle() {
            ...
            $(element).append('<div class="pageSlider-title" id="pageSlider-' + element.id + '-title" />');
            var title = getTitle();

            // append the transparency and content elements
            $(title).append('<div class="pageSlider-title-transparency" id="pageSlider-' + element.id + '-title-transparency" />');
            $(title).append('<div class="pageSlider-title-content" id="pageSlider-' + element.id + '-title-content" />');
        ...
        function initNavigation() {
            ...
            $(element).append('<div class="pageSlider-navigation" id="pageSlider-' + element.id + '-navigation" style="position: absolute" />');
            var navigation = getNavigation();

            $(navigation).css({'width': options.width, 'height': options.height, 'z-index': options.zIndex + 110})
                .append('<a class="pageSlider-navigation-previous" id="pageSlider-' + element.id + '-navigation-previous" href="javascript:void(0);" rel="prev">' + options.previousText + '</a>')
                .append('<a class="pageSlider-navigation-next" id="pageSlider-' + element.id + '-navigation-next" href="javascript:void(0);" rel="next">' + options.nextText + '</a>');

            $('a[class*="pageSlider-navigation"]', navigation).click(function () {
        ...
        function initBullets() {
            ...
            $(wrapper).append('<ol class="pageSlider-navigation-buttons" id="pageSlider-' + element.id + '-navigation-buttons" />');
            ...
            for (var i = 1; i <= count; i++) {
                ...
                $(li).append('<a class="pageSlider-navigation-button" id="pageSlider-' + element.id + '-navigation-button-' + i + '" href="javascript:void(0);" rel="' + (i - 1) + '">' + link + '</a>');
            }

            $('a[class="pageSlider-navigation-button"]', buttons).click(function () {
                clearTimeout(timeout);
                slide($(this).attr('rel'), true);
            });
        };
        ...
        function getButtonNext(asSelector) {
            var selector = '#pageSlider-' + element.id + '-navigation-next';
            if (asSelector) {
                return selector;
            }
        ...
        function getButtonPrevious(asSelector) {
            var selector = '#pageSlider-' + element.id + '-navigation-previous';
            if (asSelector) {
                return selector;
            }
            return $(selector, wrapper);
        ...
        function getNavigation(asSelector) {
            var selector = '#pageSlider-' + element.id + '-navigation';
            if (asSelector) {
                return selector;
            }
            return $(selector, wrapper);
        ...
        function getNavigationButtons(asSelector) {
            var selector = '#pageSlider-' + element.id + '-navigation-buttons';
            if (asSelector) {
                return selector;
            }
            return $(selector, wrapper);
        ...
        function getTitle(asSelector) {
            var selector = '#pageSlider-' + element.id + '-title';
            if (asSelector) {
                return selector;
            }
            return $(selector, wrapper);
        ...
        function getTitleTransparency(asSelector) {
            var selector = '#pageSlider-' + element.id + '-title-transparency';
            if (asSelector) {
                return selector;
            }
        ...
        function getTitleContent(asSelector) {
            var selector = '#pageSlider-' + element.id + '-title-content';
            if (asSelector) {
                return selector;
            }
            return $(selector, getTitle());
    ...
    $.fn.pageSlider = function (options) {
        ...
        options = $.extend(true, {}, $.pageSlider.defaults, options);

        this.each(function () {
            element = new pageSlider(this, options);
